...
The client stores the credential under a room-scoped browser key and supplies it through the Socket.IO authentication payload. A reconnect is accepted only when the token matches in constant time, the player is currently disconnected, no old socket is active, and the 120-second reconnect deadline has not expired. A successful reconnect rotates the token. Attempts to take over an active session are rejected; the server never silently replaces a connected socket.
